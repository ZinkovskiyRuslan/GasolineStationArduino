...
const int preStopTrigger = 20; //min value = 0; max value "pulsePerLiter"; 1 = 10ml
...
int waitFindWiFi = 5;
int logLevel = 2; //0 - all/ 1 - debug/2 - Error/3 - Off
...
void setup() {  
  ...
  if(digitalRead(portCheckBatteryStatus) == HIGH)
  {
    String _ = GetRestResponse("api/system/sendTelegram.php?token=klSimn53uRescojnfls&message=Переход%20АЗС%20на%20работу%20от%20сети%20220В.");   
  }
